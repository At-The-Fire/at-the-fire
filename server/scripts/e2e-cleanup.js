...
async function resolveSubByEmail(email) {
  ...
  const { rows } = await pool.query('SELECT sub FROM cognito_users WHERE email_hash = $1', [hash]);
...
async function cleanup() {
  ...
  // Seller payouts
  await dbRun('Delete seller_payouts (USER1)', 'DELETE FROM seller_payouts WHERE seller_sub = $1', [
    user1Sub,
  ]);
  ...
  await dbRun('Delete auctions (USER1)', 'DELETE FROM auctions WHERE seller_sub = $1', [user1Sub]);

  // Gallery posts — posts_imgs and likes CASCADE automatically
  await dbRun('Delete gallery_posts (USER1)', 'DELETE FROM gallery_posts WHERE seller_sub = $1', [
    user1Sub,
  ]);
  ...
  await dbRun('Delete likes (USER1 + USER2)', 'DELETE FROM likes WHERE sub = $1 OR sub = $2', [
    user1Sub,
    user2Sub,
  ]);
